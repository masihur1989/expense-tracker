...
const Expense = ({route, navigation}) => {
    const {date, categoryDatas} = route.params.data;
    const categoryListHeightAnimationValue = useRef(new Animated.Value(115)).current;

    const [categories, setCategories] = React.useState(categoryDatas)
            ...
            <View style={{ paddingHorizontal: SIZES.padding, paddingVertical: SIZES.padding, backgroundColor: COLORS.white }}>
                ...
                <View style={{ flexDirection: 'row', marginTop: SIZES.padding, alignItems: 'center' }}>
                    ...
                    <View style={{ marginLeft: SIZES.padding }}>
                        <Text style={{ color: COLORS.primary, ...FONTS.h3 }}>{moment(date).format('MMMM Do YYYY')}</Text>
    ...
    function renderIncomingExpenses() {
        ...
        let incomingExpenses = allExpenses.filter(a => a.status == "pending")
        ...
        const renderItem = ({ item, index }) => (
            <View style={{
                ...
                <View style={{ flexDirection: 'row', padding: SIZES.padding, alignItems: 'center' }}>
                    ...
                    >
                        <Image
                            source={getIcon(selectedCategory.name)}
                            style={{
                                width: 30,
                                height: 30,
                                tintColor: getColor(selectedCategory.name)
                            }}
                        />
                    </View>

                    <Text style={{ ...FONTS.h3, color: getColor(selectedCategory.name) }}>{selectedCategory.name}</Text>
                </View>
                ...
                <View
                    style={{
                        ...
                        backgroundColor: getColor(selectedCategory.name),
                    }}
                >
    ...
    function processCategoryDataToDisplay() {
        ...
        let chartData = categories.map((item) => {
            let confirmExpenses = item.expenses.filter(a => a.status == "confirmed")
    ...
    function renderChart() {
        ...
        let colorScales = chartData.map((item) => getColor(item.name))
        ...
        const renderItem = ({ item }) => (
            <TouchableOpacity
                style={{
                    ...
                    borderRadius: 10,
                    backgroundColor: (selectedCategory && selectedCategory.name == item.name) ? getColor(item.name) : COLORS.white
                }}
                onPress={() => {
                ...
                <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                    <View
                        style={{
                            width: 20,
                            height: 20,
                            backgroundColor: (selectedCategory && selectedCategory.name == item.name) ? COLORS.white : getColor(item.name),
                            borderRadius: 5
                        }}
                    />
